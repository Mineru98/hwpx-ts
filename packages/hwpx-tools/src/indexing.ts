...
import { HwpxPackage, TextExtractor } from "@ubermensch1218/hwpxcore";
...
function extractDocMeta(pkg: HwpxPackage): {
  title: string | null;
  author: string | null;
  date: string | null;
  sections: number;
  ...
  try {
    ...
    // Ignore metadata parse errors and continue indexing.
  }
...
async function readExistingRecords(
  outputPath: string,
  format: BatchIndexFormat
): Promise<Map<string, BatchIndexRecord[]>> {
    ...
    const records: BatchIndexRecord[] =
      format === "json"
        ...
        : raw
            ...
            .map((line) => line.trim())
            .filter(Boolean)
            .map((line) => JSON.parse(line) as BatchIndexRecord);

    for (const record of records) {
      const current = map.get(record.relativePath) ?? [];
      current.push(record);
      map.set(record.relativePath, current);
    }
  } catch {
    // Ignore read failures and do a full reindex for current run.
  }
...
export async function batchIndexHwpx(options: BatchIndexOptions): Promise<BatchIndexResult> {
  ...
  const previousState = options.incremental ? await readState(statePath) : emptyState;
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      const previous = previousState.files[relPath];
      const reusable =
        options.incremental &&
        !!previous &&
        previous.size === fsInfo.size &&
        previous.mtimeMs === fsInfo.mtimeMs &&
        existingRecordsByPath.has(relPath);

      if (reusable) {
        records.push(...(existingRecordsByPath.get(relPath) ?? []));
        skippedFiles += 1;
      ...
      try {
        const extractor = new TextExtractor(pkg);
        const metadata = extractDocMeta(pkg);
        const indexedAt = new Date().toISOString();
        let chunkIndex = 0;

        if (options.chunkBy === "paragraph") {
          for (const paragraph of extractor.extractParagraphs()) {
            for (const chunkText of splitByMaxChars(paragraph.text, options.maxChars)) {
              ...
              records.push({
                ...
                text: normalized,
                metadata: { ...metadata, indexedAt },
              });
              chunkIndex += 1;
            }
          }
        } else if (options.chunkBy === "section") {
          for (const [sectionIndex, sectionText] of extractor.extractSectionTexts("\n").entries()) {
            for (const chunkText of splitByMaxChars(sectionText, options.maxChars)) {
              const normalized = normalizeChunkText(chunkText);
              if (!options.includeEmpty && !normalized) continue;
          ...
          for (const chunkText of splitByMaxChars(extractor.extractText("\n"), options.maxChars)) {
            const normalized = normalizeChunkText(chunkText);
            if (!options.includeEmpty && !normalized) continue;

            records.push({
              id: toStableId([relPath, options.chunkBy, chunkIndex, normalized]),
              sourcePath: filePath,
              relativePath: relPath,
              sourceFile: basename(filePath),
              chunkBy: options.chunkBy,
